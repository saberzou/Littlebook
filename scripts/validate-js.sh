...
for file in "$@"; do
  ...
  if [ "$ext" = "js" ]; then
    ...
    # Extract inline <script> blocks (skip <script src=...> and <script type="importmap">) and check syntax
    tmpjs=$(mktemp /tmp/validate-XXXXXX.js)
    # Use awk to only extract content of <script> tags without src attribute or importmap type
    awk '
      /<script[^>]*src[[:space:]]*=/ { next }
      /<script[^>]*type[[:space:]]*=[[:space:]]*"importmap"/ { next }
